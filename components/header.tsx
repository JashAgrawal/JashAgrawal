        ...
        <ul className="flex w-[22rem] flex-wrap items-center justify-center gap-y-1 text-[0.9rem] font-medium text-gray-500 sm:w-[initial] sm:flex-nowrap sm:gap-5">
          {links.map((link) => (
            ...
            >
              <Link
                className={cn(
                  "flex w-full items-center justify-center px-3 py-3 hover:text-gray-950 transition dark:text-white dark:hover:text-gray-400",
                  activeSection === link.name
                    ? "text-gray-300 dark:text-gray-900 hover:text-gray-500"
                    : ""
                )}
                href={link.hash}
                onClick={() => {
              ...
              >
                ...
                {link.name === activeSection && (
                  <motion.span
                    className="dark:bg-gray-200 rounded-full absolute inset-0 -z-10 bg-gray-800"
